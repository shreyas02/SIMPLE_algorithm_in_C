...
void main(){
    ...
    //newman boundary conditions forward differencing 

    //a pressure point at bottom left node is taken and then pinned to 0 
    ...
    for(i=0;i<ygridsize; i++){
        dp[i][0] = dp[i][1];
        dp[i][xgridsize - 1] = dp[i][xgridsize - 2];
        newdp[i][0] = newdp[i][1];
        newdp[i][xgridsize - 1] = newdp[i][xgridsize - 2];
    }
    for(j=1;j<xgridsize + 1; j++){
        dp[0][j] = dp[1][j];
        dp[ygridsize - 1][j] = dp[ygridsize - 2][j];
        newdp[0][j] = newdp[1][j];
        newdp[ygridsize - 1][j] = newdp[ygridsize - 2][j];
    }

    do{
    for(i=1;i<ygridsize -1;i++){
        for(j=1;j<xgridsize + 1;j++){
            if (i != ygridsize-2 || j != 1){
            a = 2 * ((dt / (dx*dx)) + (dt / (dy*dy)));
            b = - dt / (dx*dx) ;
            c = - dt / (dy*dy) ;
            d = (1/dx) * ((rho*u[i][j+1]) - (rho*u[i][j]) ) + (1/dy) * ((rho*v[i-1][j]) - (rho*v[i][j])); 
    ...
    for(i = 1; i<ygridsize-1; i++){
        for(j=1; j<xgridsize ; j++){
            u[i][j] = u[i][j] - (dt/dx)*(p[i][j]-p[i][j-1]);
        }
    }
    ...
    for(i = 1; i<ygridsize; i++){
        for(j=1; j<xgridsize - 1; j++){
            v[i][j] = v[i][j] - (dt/dy)*(p[i][j]-p[i-1][j]);
        }
    }
    ...
    // printing final U 
    printf("\n");
    printf("The final U velocity is -\n");
    for(i = 0; i<ygridsize; i++){
        for(j=0; j<xgridsize + 1; j++){
            printf("%f\t",u[i][j]);
        }
    ...
    // printing final v
    printf("\n");
    printf("The final V velocity is -\n");
    for(i = 0; i<ygridsize + 1; i++){
        for(j=0; j<xgridsize; j++){
            printf("%f\t",v[i][j]);
        }
    ...
    //Printing the final pressure value p
    printf("\n");
    printf("The final Pressure is -\n");
    for(i = 0; i<ygridsize; i++){
        for(j=0; j<xgridsize; j++){
            printf("%f\t",p[i][j]);
        }
